...
def update_or_create_tokens(session_key, access_token, token_type, expires_in, refresh_token):
    ...
    if tokens:
        ...
        tokens.save(update_fields=["access_tokens", "refresh_tokens", "expires_in", "token_type"])
    else:
        tokens = SpotifyToken(
            user=session_key,
...
def is_authenticated(session_key):
    ...
    if tokens:
        ...
        if expiry <= timezone.now():
            renew_token(session_key)
        return True
    return False
        
def renew_token(session_key):
    refresh_token = get_tokens(session_key).refresh_token
...
def execute_spotify_api_call(session_id, endpoint, post_=False, put_=False):
    ...
    try:
        ...
        return {"Error": "Issue with the Spotify Request"}
